...
public class PlaytimeHistoryService : BackgroundService
{
    ...
    {
        // 启动服务后等待 30秒 运行一次
        _logger.LogInformation("⏳ 游戏时长历史记录服务已启动，将在 30秒 后执行首次数据同步...");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            _logger.LogInformation("📅 下次时长快照任务将在 {NextRun} 执行", nextRun);
    ...
    private async Task RecordPlaytimeSnapshotAsync(CancellationToken stoppingToken)
    {
        ...
        foreach (var binding in bindings)
        {
            ...
            {
                _logger.LogWarning("❌ 用户 {UserId} 未配置有效的 Steam API Key，跳过。", binding.UserId);
                continue;
            }
            ...
            try
            {
                ...
                await Task.Delay(1000, stoppingToken); // 限流
            }
            catch (Exception ex)
            {
