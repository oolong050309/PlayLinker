 ...
 * PlayLinker 数据库初始化脚本
 * 版本: 1.0.0
 * 说明: 包含5个核心Schema及所有表结构
 * 修复: 已解决索引长度限制(750)及保留字冲突(rank改为current_rank)
 */
...
CREATE SCHEMA IF NOT EXISTS game_data DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
CREATE SCHEMA IF NOT EXISTS user_library DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
CREATE SCHEMA IF NOT EXISTS business_features DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
CREATE SCHEMA IF NOT EXISTS parental_notification DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
...
CREATE TABLE game_data.platforms (
...
CREATE TABLE game_data.game_ranking (
...
CREATE TABLE game_data.game_genres (
...
CREATE TABLE game_data.developers (
...
CREATE TABLE game_data.game_developers (
...
CREATE TABLE game_data.publishers (
...
CREATE TABLE game_data.game_publishers (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    game_id BIGINT NOT NULL,
...
CREATE TABLE game_data.categories (
...
CREATE TABLE game_data.game_categories (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    game_id BIGINT NOT NULL,
...
CREATE TABLE game_data.external_links (
...
CREATE TABLE game_data.game_external_links (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    game_id BIGINT NOT NULL,
...
CREATE TABLE game_data.game_news (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    game_id BIGINT NOT NULL,
...
CREATE TABLE game_data.game_platform (
    game_id BIGINT NOT NULL,
...
CREATE TABLE user_library.player_platform (
    ...
    FOREIGN KEY (platform_id) REFERENCES game_data.platforms(platform_id)
...
CREATE TABLE user_library.user_platform_binding (
    ...
    FOREIGN KEY (user_id) REFERENCES user_management.user(user_id),
    FOREIGN KEY (platform_id) REFERENCES game_data.platforms(platform_id),
...
CREATE TABLE user_library.user_game_library (
...
CREATE TABLE user_library.user_platform_library (
    platform_user_id VARCHAR(128) NOT NULL COMMENT '平台侧用户标识（平台唯一）',
    platform_id INT NOT NULL,
    ...
    FOREIGN KEY (platform_user_id, platform_id) REFERENCES player_platform(platform_user_id, platform_id),
    FOREIGN KEY (game_id) REFERENCES game_data.games(game_id),
...
CREATE TABLE user_library.user_achievements (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES user_management.user(user_id),
    FOREIGN KEY (achievement_id) REFERENCES game_data.achievements(achievement_id),
    FOREIGN KEY (platform_id) REFERENCES game_data.platforms(platform_id),
...
CREATE TABLE user_library.local_game_install (
...
CREATE TABLE user_library.local_save_file (
...
-- 表4.3: LocalMod（本地mod）
CREATE TABLE user_library.local_mod (
...
CREATE TABLE user_library.cloud_save_backup (
...
CREATE TABLE business_features.report_template (
...
CREATE TABLE business_features.report_generation_record (
...
CREATE TABLE business_features.user_preference (
...
CREATE TABLE business_features.preference_genre (
    id INT PRIMARY KEY AUTO_INCREMENT,
    preference_id INT NOT NULL,
    genre_id INT NOT NULL,
    FOREIGN KEY (preference_id) REFERENCES user_preference(preference_id) ON DELETE CASCADE,
    FOREIGN KEY (genre_id) REFERENCES game_data.genres(genre_id),
...
CREATE TABLE business_features.recommendation (
...
CREATE TABLE business_features.recommendation_feedback (
...
CREATE TABLE business_features.price_history (
...
CREATE TABLE business_features.price_alert_subscription (
...
CREATE TABLE parental_notification.notification_center (
...
CREATE TABLE business_features.price_alert_log (
    ...
    FOREIGN KEY (notification_id) REFERENCES parental_notification.notification_center(notification_id),
...
CREATE TABLE parental_notification.parental_control_relationship (
    relationship_id INT PRIMARY KEY AUTO_INCREMENT,
    parent_user_id INT NOT NULL,
    child_user_id INT UNIQUE NOT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (parent_user_id) REFERENCES user_management.user(user_id),
    FOREIGN KEY (child_user_id) REFERENCES user_management.user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='家长监管关系表';

-- 表8.2: ParentalControlRule（家长监管规则表）
CREATE TABLE parental_notification.parental_control_rule (
...
CREATE TABLE parental_notification.parental_alert_log (
    alert_id BIGINT PRIMARY KEY AUTO_INCREMENT,
